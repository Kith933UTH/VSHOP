...
export function Payment({login}) {
    ...
    const handleSubmit = (event) => {
        const inputElements = document.querySelectorAll('.payment-page form input')
        inputElements.forEach((item) => console.log(item.value))
        console.log(event)
    }
    return (
        <div className="payment-page text-center m-5-auto flex justify-center">
            ...
            <div className="flex flex-row justify-center border-y-2">
                <form className="border-none bg-none m-0" onSubmit={handleSubmit(this)} action="/success">
                    <p>
                        <label>Name</label><br/>
