...
export function ProductDetailItem({product, refetch, status}) {
    ...
    const getColor = (option) => {
        product = {...product, color: option}
    }

    const getSize = (option) => {
        product = {...product, size: option}
    }

    const getBrand = (option) => {
        product = {...product, brand: option}
    }

    const optionsProduct = (category) => {
        if (category === 'men\'s clothing' || category === 'women\'s clothing') return <Colors option = {getColor} option2 = {getSize}/> 
        if (category === 'jewelery') return <Sizes option ={getSize}/>
        if (category === 'electronics') return <Brands  option = {getBrand}/>
    }
                ...
                <div className='flex flex-col justify-center items-center lg:flex lg:flex-row lg:justify-between lg:items-start'>
                    ...
                    <div className='w-full pl-0 sm:w-3/4 lg:w-2/4 lg:pl-[30px]'>
                        <div>
                            ...
                            <div className="flex">
                                <p className='my-4 text-xl sm:text-2xl line-through mr-4'>${product.oldPrice}</p>
